...
1. `pnpm bump <cli|client|server|shared|landing|app> <major|minor|patch>` — bumps the package AND the root app version together. CLI releases must keep `packages/cli` equal to the app version (`pnpm bump cli <level>` does this).
