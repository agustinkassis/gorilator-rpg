...
1. `pnpm bump <cli|client|server|shared|app> <major|minor|patch>` — bumps the package AND the root app version together. Package versions (including the CLI's) are independent of the app release version; CI publishes the CLI only when its version isn't already on npm.
