...
- Each package versions itself independently; the root `package.json` is the **umbrella "app" version**, which must move by **≥ the same SemVer level** whenever any package is bumped.
- To change a version: `pnpm bump <cli|client|server|shared|landing> <major|minor|patch>` — this bumps the package **and** the app together. Use `pnpm bump app <level>` for an umbrella-only catch-up.
...
- To release the CLI: `pnpm bump cli <level>` → merge to `main` → create a GitHub Release (CI publishes to npm).
