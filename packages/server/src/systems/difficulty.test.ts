...
describe("waveDifficulty (#64)", () => {
  ...
  it("defaults reproduce the legacy formulas exactly", () => {
    ...
    for (const n of [1, 2, 5, 9]) {
      const d = waveDifficulty(state, n, t);
  ...
  it("the event multiplier layers on top of the knobs", () => {
    ...
    setDevTuning("waveSizeMax", 100);
    const state = world([3]);
    expect(waveDifficulty(state, 1, devTuning(), 2).size).toBe(8);
    setDevTuning("difficultySizeScale", 2);
    expect(waveDifficulty(state, 1, devTuning(), 2).size).toBe(16); // knobs × event
    expect(waveDifficulty(state, 1, devTuning(), 0).size).toBe(8); // invalid mult → 1
  });
});
