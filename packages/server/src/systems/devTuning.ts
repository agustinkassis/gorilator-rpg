...
export const devTuningDefaults: DevTuningValues = {
  ...
  hungerDrainPerMin: HUNGER_DRAIN_PER_MIN,
  starvationDamagePerSec: STARVATION_DAMAGE_PER_SEC,
  foodHungerMult: FOOD_HUNGER_MULT,
  foodHpMult: FOOD_HP_MULT,
  foodStaminaMult: FOOD_STAMINA_MULT,
...
const rules: Record<DevTuningKey, { min: number; max: number; integer?: boolean }> = {
  ...
  hungerDrainPerMin: { min: 0, max: 600 },
  starvationDamagePerSec: { min: 0, max: 1_000 },
  foodHungerMult: { min: 0, max: 50 },
  foodHpMult: { min: 0, max: 50 },
  foodStaminaMult: { min: 0, max: 50 },
